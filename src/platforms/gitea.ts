  ...
  async createTag(options: TagOptions): Promise<TagResult> {
    ...
    const tryCreateViaRefs = async (): Promise<void> => {
      ...
      try {
        ...
        if (refsMsg.includes('405')) {
          throw new Error(
            `Gitea tag create failed: POST /repos/.../tags returned an error, and the refs fallback (POST /git/refs) is not supported by this Gitea server (405 Method Not Allowed). ` +
    ...
    try {
      ...
      if (msg.includes('409') && msg.includes('tag already exists')) {
        ...
        throw new Error(
          `Gitea tag create failed: POST /repos/.../tags returned 404 Not Found. ` +
